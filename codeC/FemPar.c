...
void Update(float *A)
{
   ...
   for(i=0;i< PROCNO; i++)
   {
	   ...
	   if (Common[i]>0)
	   {
		   MPI_Send(&Buf[0], Common[i], MPI_FLOAT, i, 1, MPI_COMM_WORLD);
		   //Send(Buf, Common[i], i))
	   }
  ...
  {	
	   if (Common[i]>0)
	   {
			MPI_Recv(&Buf[0], Common[i], MPI_FLOAT, i, 1,MPI_COMM_WORLD, &Status);
		}
